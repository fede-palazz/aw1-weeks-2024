...
export function QuestionLayout(props) {
  ...
  return (
    <>
      {/* The check on "question" is needed to:
        - intercept errors due to invalid URLs (e.g., /questions/5 when you have two questions only)
        - wait for the value of question to be loaded (at the beginning it's undefined and useEffect
          is executed AFTER the component rendering phase
        )
    */}
      {question ? (
        <>
    ...
    <>
      <Row>
        ...
        <Col md={6} as="p" className="text-end">
          Asked by{" "}
          <span className="badge rounded-pill text-bg-secondary">{props.question.email}</span>
        </Col>
      </Row>
      <Row>
