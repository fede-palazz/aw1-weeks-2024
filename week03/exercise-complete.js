...
function Question(id, text, email, date) {
  ...
  this.getTop = (num) => {
    return new Promise((resolve, reject) => {
      const sql =
        "SELECT answer.*, user.email FROM answer JOIN user ON answer.authorId=user.id WHERE answer.questionId = ? ORDER BY answer.score DESC LIMIT ?";
    ...
    return new Promise((resolve, reject) => {
      ...
      db.get(sql, [question.email], (err, row) => {
        if (err) {
          ...
          sql =
            "INSERT INTO question(text, authorId, date) VALUES(?,?,DATE(?))";
          db.run(
            sql,
...
async function main() {
  ...
  const newQuestionId = await ql.addQuestion(
    new Question(
      undefined,
      "Is 1 bigger than 10?",
