...
function Question(id, text, email, date) {
  ...
  // metodo per prendere tutte le risposte della Question istanziata
  this.getAnswers = () => {
    return new Promise((resolve, reject) => {
      const sql =
        "SELECT answer.id, text, user.email, date, score \
        FROM answer, user \
        WHERE answer.questionId=? AND answer.authorId=user.id";
