  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [answer.email], (err, row) => {
      if (err) reject(err);
        ...
        db.run(
          sql,
          [answer.text, row.id, answer.date, answer.score, questionId],
          function (err) {
            if (err) reject(err);
            else resolve(this.lastID);
          }
  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(sql, [answer.email], (err, row) => {
      if (err) reject(err);
        ...
        sql =
          "UPDATE answer SET text = ?, authorId = ?, date = DATE(?), score = ? WHERE id = ?";
        db.run(
          sql,
          [answer.text, row.id, answer.date, answer.score, answer.id],
          function (err) {
            if (err) reject(err);
            else resolve(this.lastID);
          }
